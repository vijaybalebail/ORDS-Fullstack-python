...
def home():
    ...
    """
    client_id1 = "e4meQ53cXGq53j6uffdULVjRl8It8M3FVsupKei0nSg"
    client_secret1 = "a2a67c7c-fe6c-4289-99f3-670d1d30275f"
    encodedData = base64.b64encode(bytes(f"{client_id1}:{client_secret1}", "ISO-8859-1")).decode("ascii")
   #authorization_header_string = f"Authorization: Basic {encodedData}"
    ...
    print(data)
    response1 = requests.post(options['BaseUrl'] + '/oauth2/v1/introspect?', data=data, headers=headers)
                            # auth=(options['ClientId1'], options['ClientSecret1']))
    
    print(response1.json().get("active"))
...
def restapi():
    ...
    #id_token_verified = am.verifyToken(id_token)
    #id_token_verified = am.verifyIdToken(id_token)
   ...
   # print( " is the auth_token_response or ords\n")
    ...
    #headers = {"Authorization": "Bearer 3YG18FubSGHRj29_BXABlA"}
